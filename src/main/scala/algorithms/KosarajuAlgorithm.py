...
class Solution:
    def maxNumOfSubstrings(self, s):
        ...
        maxs = [-math.inf] * 26
        exists = [False] * 26
        # consider every substring from 0 to i,mark what char is represented in it.
        ...
        for i in range(len(s)):
            ...
            mins[code(s[i])] = min(i, mins[code(s[i])])
            maxs[code(s[i])] = max(i, maxs[code(s[i])])
        ...
        for i in range(26):
            if exists[i]:
                for j in range(26):
                    ...
                    # between i and j
                    if (pr_sum[maxs[i] + 1][j] - pr_sum[mins[i]][j]) > 0:
                        graph[i][j] = True
        ...
        for i in range(batch - 1, -1, -1):
            ...
            if degree[i] == 0:
                ...
                for j in range(26):
                    if batches[j] == i:
                        min_v = min(min_v, mins[j])
                        max_v = max(max_v, maxs[j])
